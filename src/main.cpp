...
uint256 hashGenesisBlock("0x000007137148271ed17b6f6e25fd4056565e43b49aafa9d9538feb2527765d69"); //mainnet

static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Tomatocoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Tomatocoin Signed Message:\n";
...
{
    // Tomatocoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
static const int64 nTargetTimespan = 16 * 60 * 60; // Tomatocoin: 16 hours
static const int64 nTargetSpacing = 2 * 60; // Tomatocoin: 2 minutes
...
unsigned int static GetNextWorkRequired_V1(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Tomatocoin: This fixes an issue where a 51% attack can change difficulty at will.
...
unsigned int static DarkGravityWave(const CBlockIndex* pindexLast, const CBlockHeader *pblock) {
    /* current difficulty formula, Tomatocoin - DarkGravity v2, written by Evan Duffield - evan@Tomatocoin.io */
    const CBlockIndex *BlockLastSolved = pindexLast;
    const CBlockIndex *BlockReading = pindexLast;
    const CBlockHeader *BlockCreating = pblock;
    BlockCreating = BlockCreating;
...
unsigned int static DarkGravityWave3(const CBlockIndex* pindexLast, const CBlockHeader *pblock) {
    /* current difficulty formula, Tomatocoin - DarkGravity v3, written by Evan Duffield - evan@Tomatocoin.io */
    const CBlockIndex *BlockLastSolved = pindexLast;
    const CBlockIndex *BlockReading = pindexLast;
    const CBlockHeader *BlockCreating = pblock;
    BlockCreating = BlockCreating;
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckVotes) const
{
    ...
    // Tomatocoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // Tomatocoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        block.nVersion = 1;
        block.nTime    = 1401256800;
        block.nBits    = 0x1e0fffff;
        block.nNonce   = 452911;

#if 0
        unsigned int t = bnProofOfWorkLimit.GetCompact();
        printf("t = %x\n", t);
        uint256 h = CBigNum().SetCompact(t).getuint256();
        printf("h = %s\n", h.ToString().c_str());
        exit(0);
#endif
        //nNonce
        ...
        printf("loop %ld times, found :%u\n", nHashesDone, block.nNonce);
        exit(0);
#endif
        //hash
#if 0
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("TomatocoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("TomatocoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static TomatocoinMiner(CWallet *pwallet)
{
    printf("TomatocoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("Tomatocoin-miner");
    ...
    try { loop {
        ...
        printf("Running TomatocoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("TomatocoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&TomatocoinMiner, pwallet));
}
...
/* *** BEGIN DARKSEND MAGIC - Tomatocoin **********
    Copyright 2014, Tomatocoin Developers 
        eduffield - evan@Tomatocoin.io
        InternetApe - kyle@Tomatocoin.io
*/
